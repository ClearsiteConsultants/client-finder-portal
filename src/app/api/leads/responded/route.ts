...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      assertValidTransition(business.leadStatus, 'responded');
    } catch (error) {
      if (error instanceof InvalidStateTransitionError) {
        return NextResponse.json({
          error: 'Invalid state transition',
          currentStatus: business.leadStatus,
          message: `Cannot mark lead as responded from ${business.leadStatus} state`,
        }, { status: 400 });
      }
      throw error;
    }
    ...
    const updated = await prisma.business.update({
      ...
      data: {
        ...
        notes: note ? `${note}\n---\nMarked as responded at ${new Date().toISOString()}` : `Marked as responded at ${new Date().toISOString()}`,
      },
    });
